    ...
    <div>
        <div className='flex flex-wrap justify-around gap-0.5'>
            {characters.map((character) => (
                <div key={character.id} className='w-[150px] min-h-[140px] border-2 p-2 mb-2 rounded'>
                    <div className='mb-2'>
                    ...
                    <Link href={`/character/${character.id}`} className='text-white bg-slate-500 border-2 rounded p-1 text-sm'>
                    {en.readMore}
                    </Link>
                </div>
            ))}
